...
const MentorDashboard = () => {
  ...
  const fetchUsers = React.useCallback(
    async ({ stage, filter }: FetchUsersProps): Promise<void> => {
      setLoading(true);
      try {
        const response = await fetch(
          `/api/mentor/users?stage=${encodeURIComponent(stage)}&filter=${filter}`
        );
        if (!response.ok) {
      ...
      return (
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2">
            ...
            <span>
              {formatMeetingDate(user.nextMeeting.date)} at{' '}
              {user.nextMeeting.time}
            </span>
          </div>
  ...
  return (
    <div className="container mx-auto p-6 pt-28">
      ...
      <Tabs
        defaultValue="stage3"
        value={selectedTab}
        onValueChange={(value) => setSelectedTab(value as TabStage)}
      >
                        ...
                        <TableRow key={user.id}>
                          ...
                          <TableCell className="flex gap-2">
                            ...
                            >
                              ...
                              {user.nextMeeting && !user.nextMeeting.isCompleted
                                ? 'Reschedule'
                                : 'Schedule'}
                            </Button>
                          </TableCell>
