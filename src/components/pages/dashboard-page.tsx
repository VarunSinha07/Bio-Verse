...
export default function DashboardPage() {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            if (response.ok) {
              ...
              // Check for meeting info if stage is 3
              if (stage === 3) {
                try {
                  ...
                  if (meetingResponse.ok) {
                    ...
                    // Handle the case where meetingData is an array
                    const meetingInfo =
                      Array.isArray(meetingData) && meetingData.length > 0 ? meetingData[0] : meetingData

                    // Update dashboard data with meeting info
                    ...
                    console.error("Failed to fetch meeting info:", await meetingResponse.text())
                  }
                } catch (meetingError) {
            ...
            if (Number.parseInt(data.currentStage) === 3) {
              try {
                const meetingResponse = await fetch("/api/meeting-info")
    ...
    try {
      ...
      setDashboardData((prev) => {
        ...
        const documents = {
          ...
          signedNDA: uploadedDocuments.signedNDA ? `${userId}-signedNDA.pdf` : prev.documents?.signedNDA,
        }
      ...
      toast({
        title: "Error",
        description: "Failed to submit business plan and documents. Please try again.",
        variant: "destructive",
        duration: 5000,
      })
    } finally {
  ...
  const handleSendRescheduleEmail = () => {
    if (dashboardData?.meeting?.mentorEmail) {
      const subject = encodeURIComponent("Request to Reschedule Mentor Meeting")
      ...
      toast({
        ...
        description: "You can now send your rescheduling request to the mentor.",
        duration: 3000,
      })
    }
  }
          ...
          <div className="flex items-center justify-center gap-2 md:gap-4 p-6 bg-white/90 rounded-xl shadow-md w-full max-w-3xl">
            {[1, 2, 3, 4].map((stage) => (
              ...
              >
                ...
                <div className="text-center mt-2 text-sm font-medium">Stage {stage}</div>
                {stage < 4 && (
                  <div
          ...
          >
            {businessPlan ? (
              ...
              <Card className="bg-white/90 backdrop-blur-sm shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100">
                ...
                <CardContent className="space-y-4 p-6">
                  <h3 className="font-semibold text-lg text-green-700 mb-4">Business Plan Information</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <DetailItem
                      icon={<BookOpen />}
                  ...
                  <h3 className="font-semibold text-lg text-green-700 mt-6 mb-4">Uploaded Documents</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <DocumentItem
                      title="Competitor Analysis"
                      isUploaded={!!dashboardData.documents?.competitorAnalysis}
                    />
                    <DocumentItem title="Founders' Profiles" isUploaded={!!dashboardData.documents?.foundersProfile} />
                    <DocumentItem
                    ...
                    />
                    <DocumentItem title="Signed NDA" isUploaded={!!dashboardData.documents?.signedNDA} />
                  </div>
                </CardContent>
              </Card>
            ) : (
                ...
                <CardContent>
                  <form onSubmit={handleSubmitStage2} className="space-y-6">
                    <div className="space-y-4">
                      <h3 className="font-semibold text-lg text-green-700">Business Plan Information</h3>
                    ...
                    <div className="space-y-4">
                      <h3 className="font-semibold text-lg text-green-700">Required Documents</h3>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <DocumentUpload
                          ...
                          handleFileChange={(e) => handleFileChange(e, "competitorAnalysis")}
                          handleRemoveFile={() => handleRemoveFile("competitorAnalysis")}
                          required={true}
                        />
                        ...
                        <DocumentUpload
                          ...
                          handleFileChange={(e) => handleFileChange(e, "foundersProfile")}
                          handleRemoveFile={() => handleRemoveFile("foundersProfile")}
                          required={true}
                        />
                        ...
                        <DocumentUpload
                          ...
                          handleFileChange={(e) => handleFileChange(e, "productMockups")}
                          handleRemoveFile={() => handleRemoveFile("productMockups")}
                          required={true}
                        />
                        ...
                        <DocumentUpload
                          ...
                          handleFileChange={(e) => handleFileChange(e, "marketResearch")}
                          handleRemoveFile={() => handleRemoveFile("marketResearch")}
                          required={true}
                        />
                        ...
                        <DocumentUpload
                          ...
                          handleFileChange={(e) => handleFileChange(e, "signedNDA")}
              ...
              <CardContent className="space-y-6 p-6">
                {dashboardData.meeting ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
                          ...
                          <div className="text-lg font-semibold">
                            {dashboardData.meeting?.mentorName || "Not assigned"}
                          </div>
                          <div className="text-gray-500 mt-1 text-sm">
                            {dashboardData.meeting?.mentorEmail || "Email not available"}
                          </div>
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col">
                          ...
                          <div className="text-gray-500 mt-1 text-sm">{formatTime(dashboardData.meeting?.time)}</div>
                        </div>
                      </div>
                    ...
                    <div className="p-6 bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 rounded-xl shadow-sm">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 shadow-sm flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="font-medium text-gray-800">Review Business Plan</h4>
                            <p className="text-gray-600 text-sm mt-1">
                              Refresh your memory on key points before the meeting
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 shadow-sm flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="font-medium text-gray-800">Prepare Questions</h4>
                            <p className="text-gray-600 text-sm mt-1">List specific questions for your mentor</p>
                          </div>
                        </div>
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 shadow-sm flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="font-medium text-gray-800">Progress Report</h4>
                            <p className="text-gray-600 text-sm mt-1">
                        ...
                        <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 shadow-sm flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="font-medium text-gray-800">Tech Check</h4>
                            <p className="text-gray-600 text-sm mt-1">
                  ...
                  <div className="flex flex-col items-center justify-center p-8 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-700 mb-3">Mentor Session Coming Soon</h3>
                    <p className="text-gray-500 max-w-md">
                      Your mentor session is being scheduled. Once confirmed, the meeting details will appear here.
                    </p>
                  </div>
                )}
              </CardContent>
